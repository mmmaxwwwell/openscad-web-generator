...
- [x] **4.1** Create `src/lib/openscad-worker.ts` — Web Worker that loads OpenSCAD WASM
- [x] **4.2** Create `src/lib/openscad-api.ts` — main-thread API: postMessage calls, promise wrappers
- [x] **4.3** Create `src/hooks/useOpenSCAD.ts` — React hook wrapping the API (render, preview, status)
- [x] **4.4** Implement parameter injection — prepend user param values to scad source before sending to WASM (in openscad-api.ts, with 9 unit tests)
...
- `tsc --noEmit` still passes after Phase 4, all 36 tests pass (27 parser + 9 API)
- OpenSCAD WASM `openscad.js` uses `import.meta.url` and dynamic `import()` internally — works with Vite's module workers
- Worker uses `new Worker(new URL('./openscad-worker.ts', import.meta.url), { type: 'module' })` for Vite compatibility
- `callMain()` returns exit code; stdout/stderr captured via Emscripten `print`/`printErr` callbacks
- Camera arg reordering: file format is rot,trans,dist but CLI expects trans,rot,dist — handled in `viewpointToCameraArg()`
