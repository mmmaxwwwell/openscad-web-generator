...
function send(fn, data, onreply, zerocopy) {
    ...
    console.log(`[kiri-client] send task="${fn}" seq=${seq}`);
...
export const client = {
    ...
    // Allow injecting a pre-created Worker (for Vite bundling compatibility)
    ...
    restart() {
        ...
        worker.onerror = (error) => {
            console.error('[kiri-client] worker error:', error);
            // notify all pending callbacks about the error
            for (let key in running) {
                let rec = running[key];
                if (rec.fn) rec.fn({error: `worker error: ${error.message || error}`});
            }
        };
        ...
        client.onmessage = worker.onmessage = (e) => {
            ...
            console.log(`[kiri-client] recv task="${reply.task}" seq=${reply.seq} done=${reply.done}`, reply.data ? Object.keys(reply.data) : 'no-data');
            if (reply.done) {
