...
const QR_URL = 'https://raw.githubusercontent.com/xypwn/scadqr/main/qr.scad';
...
function downloadQR() {
  const tmp = join(tmpdir(), 'qr-download');
  mkdirSync(tmp, { recursive: true });
  const dest = join(tmp, 'qr.scad');
  console.log('qr.scad not found locally, downloading from GitHub...');
  execSync(`curl -sL "${QR_URL}" -o "${dest}"`, { stdio: 'inherit' });
  if (!existsSync(dest)) {
    throw new Error('Failed to download qr.scad');
  }
  return dest;
}
