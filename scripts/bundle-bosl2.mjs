...
const BOSL2_REPO = 'https://github.com/BelfrySCAD/BOSL2/archive/refs/heads/master.tar.gz';
...
function downloadBOSL2() {
  const tmp = join(tmpdir(), 'bosl2-download');
  mkdirSync(tmp, { recursive: true });
  console.log('BOSL2 not found locally, downloading from GitHub...');
  execSync(`curl -sL "${BOSL2_REPO}" | tar xz -C "${tmp}"`, { stdio: 'inherit' });
  const extracted = join(tmp, 'BOSL2-master');
  if (!existsSync(join(extracted, 'std.scad'))) {
    throw new Error('Downloaded BOSL2 archive does not contain std.scad');
  }
  return extracted;
}
...
async function main() {
  ...
  let bosl2Path = findBOSL2();
  if (!bosl2Path) {
    bosl2Path = downloadBOSL2();
  }
